...
## Managing Issued Licenses via CLI

Products are the tiers you sell; **licenses** are the individual keys issued to customers. Most licenses are created automatically by the Stripe integration, but you can also issue, list, and revoke them by hand with `amore licenses`, useful for comps, support, and manual sales.

### List Licenses

```sh
amore licenses list --bundle-id com.example.App
amore licenses list -b com.example.App --email alice@example.com --status active
```

Filters: `--product <uuid>`, `--email <substring>`, `--status <status>`.

### Issue a License

```sh
amore licenses issue -b com.example.App --product <product-id> \
  --email friend@example.com --never-expires
```

- `--product`: Product ID (UUID) to issue against (required)
- `--email`: Customer email (optional)
- `--expires <yyyy-MM-dd>`: Expiry date, or use `--never-expires` for perpetual (mutually exclusive)
- `--send-email`: Email the key to the customer (requires `--email`)

### Revoke a License

```sh
amore licenses revoke <license-id> -b com.example.App
amore licenses revoke <license-id> -b com.example.App --activation <activation-id>
```

Revoking without `--activation` deletes the license and deactivates all its devices. Pass `--activation <uuid>` to free up a single device instead. Add `--yes` to skip the confirmation prompt.
