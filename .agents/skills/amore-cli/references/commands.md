...
# Non-interactive local release with explicit credentials
amore release --scheme MyApp \
...
### `amore export sparkle-key --bundle-id <id>`
Print the base64-encoded Sparkle Ed25519 private key from the keychain to stdout. Use it to populate the `SPARKLE_PRIVATE_KEY` secret for the [GitHub Action](../SKILL.md#cicd-with-github-actions). Falls back to the legacy (non-per-app) key if no per-app key exists.

```sh
amore export sparkle-key -b com.example.App | pbcopy
```
...
## License Management (Issued Licenses)

Products are the tiers you sell; licenses are the individual keys issued to customers. Manage issued licenses with `amore licenses`.

### `amore licenses list --bundle-id <id> [options]`
List issued licenses for an app.

**Options:**
- `--product <uuid>`: Filter by product ID
- `--email <substring>`: Filter by customer email
- `--status <status>`: Filter by status (e.g. `active`)

### `amore licenses issue --bundle-id <id> --product <uuid> [options]`
Issue a new license for a product.

**Options:**
- `--email <email>`: Customer email
- `--expires <yyyy-MM-dd>`: Expiry date (mutually exclusive with `--never-expires`)
- `--never-expires`: Issue a perpetual license
- `--send-email`: Email the key to the customer (requires `--email`)

### `amore licenses revoke <license-id> --bundle-id <id> [options]`
Revoke a license, or a single activated device.

**Options:**
- `--activation <uuid>`: Revoke one device instead of the whole license
